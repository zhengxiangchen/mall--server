function Tag(inputId){
	...
	obj.initView=function(){
		...
		appendStr+='<div class="tagList"></div><input type="text" class="tagInput2"/>';
		appendStr+='</div>';
		inputObj.after(appendStr);
		var tagInput2 = $("#"+inputId+"_tagcontaine .tagInput2");
		if(!this.isDisable){
			$("#"+inputId+"_tagcontaine").attr("ds","1");
			tagInput2.keydown(function(event){
				if(event.keyCode==13){
	...
	obj.unDisableFun = function(){
		...
		tagContaine.append('<input type="text" class="tagInput2"/>');
