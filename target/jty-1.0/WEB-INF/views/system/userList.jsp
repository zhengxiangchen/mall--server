...
<div id="tb" style="padding:5px;height:auto">
        <div>
        	<form id="searchFrom" action="">
        		<input type="text" name="filter_LIKES_loginName" class="easyui-validatebox" data-options="width:150,prompt: '用户名'"/>
	        ...
	        <shiro:hasPermission name="sys:user:disable">
	            <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" data-options="disabled:false" onclick="disable()">禁用</a>
	        	<span class="toolbar-item dialog-tool-separator"></span>
	        </shiro:hasPermission>
	        <shiro:hasPermission name="sys:user:disable">
	            <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" plain="true" data-options="disabled:false" onclick="reable()">恢复</a>
	        	<span class="toolbar-item dialog-tool-separator"></span>
	        </shiro:hasPermission>
...
function disable(){
	var row = dg.datagrid('getSelected');
	if(rowIsNull(row)) return;
	parent.$.messager.confirm('提示', '是否禁用？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/system/user/disable/"+row.id,
				success: function(data){
					successTip(data,dg);
				}
			});
...
function reable(){
	var row = dg.datagrid('getSelected');
	if(rowIsNull(row)) return;
	parent.$.messager.confirm('提示', '是否恢复正常？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/system/user/reable/"+row.id,
				success: function(data){
					successTip(data,dg);
				}
			});
